...
def add_line_gt(matrix, add_row=True):
    if add_row:
        rows = torch.zeros(matrix.shape[0], matrix.shape[1], 1).cuda()
        sum_row = torch.sum(matrix.float(), dim=2, keepdim=True)
        rows[sum_row == 1.0] = 1.0
        return torch.cat([matrix.float(), rows], dim=2)

    else:
        columns = torch.zeros(matrix.shape[0], 1, matrix.shape[2]).cuda()
        sum_col = torch.sum(matrix.float(), dim=1, keepdim=True)
        columns[sum_col == 1.0] = 1.0
        return torch.cat([matrix.float(), columns], dim=1)


def add_line_predt (matrix, add_row=True, delta=0.5):
        if add_row:
            rows = torch.zeros(matrix.shape[0], matrix.shape[1], 1).cuda() + delta
            return F.softmax(torch.cat([matrix, rows], dim=2), dim=2)

        else:
            columns = torch.zeros(matrix.shape[0], 1, matrix.shape[2]).cuda() + delta
            return F.softmax(torch.cat([matrix, columns], dim=1), dim=1)
...
def prepare_Data(data_pth, train=True, dataset='all', disttype='all'):
    """
    :param data_pth: string that gives the data path
    :return: data list
    """
    dist_types = ["giou", "l2IOU", "l2"]
    datasets = ['mot16', 'mot19', 'mot17']
    if dataset in datasets:
        datasets = [dataset]
    if disttype in dist_types:
        dist_types = [disttype]
    data = list()
    for dset in datasets:
        subset_pth = os.path.join(data_pth, dset)
        for dist_type in dist_types:
            if train:
                # print(os.path.join(subset_pth, 'train/' + dist_type))
                d_pth = os.path.join(subset_pth, 'train/' + dist_type)
            else:
                # print(os.path.join(subset_pth, 'test/' + dist_type))
                d_pth = os.path.join(subset_pth, 'test/' + dist_type)
...
class RealData(data.Dataset):
  ...
  def __init__(self, data_path, train=True, dataset='all', disttype='all'):
        'Initialization'
        self.data_pth = data_path
        self.data = prepare_Data(data_path, train, dataset, disttype)
...
def main(model, tst_dataloader, args):
    ...
    for data, target in tst_dataloader:
        ...
        if not args.use_sigmoid:
            A_1 = add_line_predt(tag_scores, add_row=True, delta=0.5)
            A_1_hat = A_1[0:, 0:, :- 1]
            A_2 = add_line_predt(tag_scores, add_row=False, delta=0.5)
            A_2_hat = A_2[0:, :- 1, 0:]

            tag_scores = torch.max(A_1_hat, A_2_hat)
    ...
    print('many ones wrong lines rate: ', sum_many_ones / count_lines)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_root', dest='data_root',
                        default='/home/yixu/Downloads/inbatch8/inBatches8/',
                        help='dataset root path')

    parser.add_argument('--is_cuda', dest='is_cuda', default=True, type=bool, help='use GPU?')
    parser.add_argument('--use_sigmoid', dest='use_sigmoid', default=False, type=bool, help='use sigmoid?')

    parser.add_argument('--model_path', dest='model_path',
                        default="/home/yixu/Downloads/DHNmodels/20190702_focalLossPlusConstraints_alldata_mean/DHN_2_.pth",
    ...
    parser.add_argument('--row_wise', dest='row_wise', default=True, type=bool, help='row wise soft-max? column wise '
                                                                                     'if False')
    parser.add_argument('--threshold', dest='threshold', default=0.4, type=float, help='threshold for dicretization.')
    ...
    model = Munkrs(element_dim=args.element_dim, hidden_dim=args.hidden_dim, target_size=args.target_size,
                   biDirenction=args.bidrectional, minibatch=args.batch_size, is_cuda=args.is_cuda, is_train=False, sigmoid=args.use_sigmoid)
    model.eval()
    if args.is_cuda:
        model.cuda()
    ...
    tst_dataset = RealData(args.data_root, train=False)
